...
public class TasksApiController implements TasksApi {
    ...
    public ResponseEntity<Object> cancelTask(@Parameter(description = "", required = true) @PathVariable("id") String id) {
    ...
    public ResponseEntity<TesCreateTaskResponse> createTask(@Parameter(description = "", required = true) @Valid @RequestBody TesTask body) {
        TesCreateTaskResponse response = this.tesService.createTask(body, this.getUser());
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    public ResponseEntity<TesTask> getTask(@Parameter(description = "", required = true) @PathVariable("id") String id,
                                           @Parameter(description = "OPTIONAL. Affects the fields included in the returned Task messages. See TaskView below.   - MINIMAL: Task message will include ONLY the fields:   Task.Id   Task.State  - BASIC: Task message will include all fields EXCEPT:   Task.ExecutorLog.stdout   Task.ExecutorLog.stderr   Input.content   TaskLog.system_logs  - FULL: Task message includes all fields.",
                                                      allowableValues = "MINIMAL, BASIC, FULL", defaultValue = "MINIMAL")
                                           @RequestParam(value = "view", required = false, defaultValue = "MINIMAL") String view) {
    ...
    public ResponseEntity<TesListTasksResponse> listTasks(@Parameter(description = "OPTIONAL. Filter the list to include tasks where the name matches this prefix. If unspecified, no task name filtering is done.") @RequestParam(value = "name_prefix", required = false) String namePrefix,
                @Parameter(example="256", description = "OPTIONAL. Number of tasks to return in one page. Must be less than 2048. Defaults to 256.") @RequestParam(value = "page_size", required = false) Long pageSize,
                @Parameter(description = "OPTIONAL. Page token is used to retrieve the next page of results. If unspecified, returns the first page of results. See ListTasksResponse.next_page_token") @RequestParam(value = "page_token", required = false) String pageToken,
