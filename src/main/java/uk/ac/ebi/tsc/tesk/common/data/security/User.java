package uk.ac.ebi.tsc.tesk.common.data.security;
...
public class User implements Serializable, UserDetails, Principal {
     ...
     * (see {@link uk.ac.ebi.tsc.tesk.config.security.ElixirPrincipalExtractor} to see, how that information is retrieved from allGroups)
    ...
    /**
     * List of 'organisational' groups, where the user is an admin (see {@link uk.ac.ebi.tsc.tesk.config.security.ElixirPrincipalExtractor} to see, how that information is retrieved from allGroups)
     * TES task will be 'owned' by one of the groups, to which the creating user belongs (or by no group, if the user does not belong to any)
     */
    ...
    public String getLabelSelector(String groupNameLabelKey, String userIdLabelKey) {
        ...
        if (isMember() || isManager()) {
            ...
            sb.append(groupNameLabelKey).append(" in (").append(StringUtils.collectionToCommaDelimitedString(allTeskGroups)).append(")");
            if (!isManager()) {
                sb.append(",").append(userIdLabelKey).append("=").append(getUsername());
            }
            return sb.toString();
        }
        return null;
