...
import uk.ac.ebi.tsc.tesk.common.data.tes.TesState;
import uk.ac.ebi.tsc.tesk.common.data.tes.TesTask;
import uk.ac.ebi.tsc.tesk.k8s.config.GsonConfig;
import uk.ac.ebi.tsc.tesk.k8s.config.KubernetesObjectsSupplier;
import uk.ac.ebi.tsc.tesk.k8s.config.TaskmasterEnvProperties;
import uk.ac.ebi.tsc.tesk.k8s.constant.Constants;
import uk.ac.ebi.tsc.tesk.k8s.data.TaskBuilder;
import uk.ac.ebi.tsc.tesk.k8s.util.JobNameGenerator;
...
public class TesKubernetesConverterTest {
    ...
    public void fromTesTaskToK8sJob() throws IOException {
        ...
        taskMasterInputJson.extractingJsonPathArrayValue("executors[*].spec.template.spec.containers[0].resources.requests.memory").containsOnly("15G").hasSize(2);
